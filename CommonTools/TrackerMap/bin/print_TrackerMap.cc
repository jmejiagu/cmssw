...
void printTrackerMap(const std::string filename, const std::string title, const std::string outfile, const int size, const std::string logscale, std::string withpixel, const float min, const float max, bool is_4_cols = false);
...
int main(int argc, char *argv[]) 
{
	if (argc >= 4) 
	{
		...
		printTrackerMap (filename, title, outfile, size, logscale, withpixel, min, max, is_4_cols);
	}
	else 
	{ 
		std::cout << "Wrong number of arguments: " << argc << std::endl; return -1; 
	}

	return 0;
}

void printTrackerMap(const std::string filename, const std::string title, const std::string outfile, const int size, const std::string logscale, std::string withpixel, const float min, const float max, bool is_4_cols) 
{
	...
	if (is_4_cols)
	{
		while ( input >> detid >> colorr >> colorg >> colorb) 
		{
			themap.fillc(detid, colorr, colorg, colorb);
		}
	}
	else
	{
